...
void window_setVSync(uint id, bool vsync) {
	auto wnd = getWindow(id);
	wnd->vsync = vsync;
	setCurrWindowContext(wnd);
	glfwWindowHint(GL_DOUBLEBUFFER, vsync);
}
...
void window_setMousePosition(uint id, int x, int y) {
	glfwSetCursorPos(getWindow(id)->handle, x, y);
}
